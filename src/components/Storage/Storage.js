...
function Storage({ web3, nId, account }) {
    ...
    let Storage =  new web3.eth.Contract(storageContract.abi, storageContract.networks[nId]?.address);
    
    // let Mine = new web3.eth.Contract(mineContract.abi, mineContract.networks[nId]?.address); 
   useEffect (( ) => {
   ...
   const ThisIsMine =  async () => {
    ...
    await window.ethereum.request({method:'eth_sendTransaction',params:[txparam]}).then(function(txparam) {
      console.log(txparam)
    })
    console.log()
  }
